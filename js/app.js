...
function validarFormulario(e){
    ...
    if(terminoBusqueda === ''){
        mostrarAlerta('Debe ingresar un termino de busqueda');
        return;
    }
    terminoBusqueda = terminoBusqueda.replace(/ /g,'+');
    buscarImagenes();
}

function mostrarAlerta(mensaje){
    const existeAlerta = formulario.querySelector('.bg-red-100');
    if(existeAlerta)
        return;
    const alerta = document.createElement('p');
    alerta.className = `bg-red-100 border-red-400 text-red-700 px-4 py-3
...
function buscarImagenes(){
    let termino = terminoBusqueda;
    const key = '25952892-cdfd1a377f461a42515e3f958'
